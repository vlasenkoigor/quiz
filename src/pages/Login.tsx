...
import { useAuth } from '../services/auth/auth-firebase';
  ...
  return (
    <>
      <Container maxWidth="xs" sx={{ flexGrow: 1, display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
        ...
        >
          <Typography variant="h4">{mode === 'signIn' ? 'Welcome back' : 'Welcome'}</Typography>
          <Grid container spacing={1.5} justifyContent={'center'} sx={{ mt: 1 }}>
            <Grid item xs={8}>
              <Button
                onClick={() => signInWithProvider('google')}
                color={'white'}
                sx={{ fontWeight: 'bold', textTransform: 'none', overflow: 'hidden' }}
                fullWidth
                variant={'contained'}
            ...
            <Grid item xs={8} sx={{ display: 'flex', flexDirection: 'column' }}>
              ...
              />
              {mode === 'signIn' && (
                <Link component={RouterLink} paddingY={0.3} align={'right'} to="/passowrd-recovery" variant="caption">
                  Forgot password?
                </Link>
              )}
            ...
            {mode === 'signIn' && (
              <Grid item xs={8}>
                <Typography align={'center'}>
                  ...
                  <Link component={RouterLink} paddingTop={0.3} align={'right'} to="/sign-up">
                    Sign up
                  </Link>
                </Typography>
              </Grid>
            ...
            {mode === 'signUp' && (
              <Grid item xs={8}>
                <Typography align={'center'}>
                  ...
                  <Link component={RouterLink} paddingTop={0.3} align={'right'} to="/sign-in">
                    Sign up
                  </Link>
                </Typography>
              </Grid>
