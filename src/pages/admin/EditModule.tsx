...
import SelectPicture from '../../components/shared/SelectPicture';
import { TModule } from '@/data/module/module-api';
import PublishWithdrawButton from '@/components/admin/shared/publish-withdraw-button';
import { useAdminModule } from '@/providers/admin-module-provider';
...
import { useRemoveWithConfirmation } from '@/hooks/admin/use-remove-with-confirmation';
...
function EditModule() {
  ...
  const { publishOrWithdraw } = usePublishWithdrawWithConfirmation(
          ...
          <Stack spacing={2} direction="row" sx={{ mt: 2 }}>
            {lessons.map((lesson, index) => (
              <Card sx={{ maxWidth: 345 }} key={index}>
                <LessonCard lesson={lesson} completed={false} />

                <CardActions>
                  <Button size="small" color="primary" component={RouterLink} to={`lessons/${lesson.id}`}>
                    Edit
                  </Button>
...
const AddQuizDialog: React.FC<{
  ...
  return (
    <Dialog open={isOpen} keepMounted onClose={handleClose} aria-describedby="alert-dialog-slide-description">
      ...
      <DialogActions sx={{ justifyContent: 'center' }}>
        <Stack direction="row" spacing={2}>
          <Button disabled={isSaving} onClick={handleClose} variant="contained" color="error">
            Cancel
          </Button>
          <Button disabled={isSaving} onClick={handleSave} variant="contained" color="success">
            Add quiz
          </Button>
        </Stack>
      </DialogActions>
