...
import { doc, onSnapshot, setDoc, updateDoc, deleteDoc, getDoc, DocumentReference } from 'firebase/firestore';
...
export function useProgress() {
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(playerProgressDocRef.current, (doc) => {
      if (doc.exists()) {
        ...
        createPlayerProgress({ playerId }).then((data) => setProgress(data));
      }
    });
  ...
  const completeLesson = useCallback(
    async (moduleID: string, lessonId: string) => {
      ...
      let lessonIndex =
        progress.modules[moduleIndex].lessons?.findIndex((lesson) => lesson.lessonId === lessonId) ?? -1;
...
export function isLessonCompleted(progress: Partial<TProgress>, moduleID: string, lessonId: string) {
  return !!progress.modules
    ...
    ?.lessons?.find((lesson) => lesson.lessonId === lessonId)?.completed;
}
